...
Printer.prototype.println = function(content){
  return this.print([ content, commands.EOL ].join(''));
};
...
Printer.prototype.feed = function (n) {
  this.buffer.write(new Array(n || 1).fill(commands.EOL).join(''));
  return this.flush();
};
...
Printer.prototype.size = function(width, height){
  ...
  if(width == 2)  this.buffer.write(commands.TEXT_FORMAT.TXT_2WIDTH);
  if(height == 2) this.buffer.write(commands.TEXT_FORMAT.TXT_2HEIGHT);
  return this;
};
